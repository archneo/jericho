...
from utils.models import ThemeSave
...
async def list_themes(request: Request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, name, description, category, tokens, fontFamily, effects, created, updated FROM themes ORDER BY updated DESC"
    )
    rows = c.fetchall()
    conn.close()
    return [
        {
            ...
            "fontFamily": r[5],
            "effects": json.loads(r[6]) if r[6] else {},
            "created": r[7],
            "updated": r[8],
        }
        for r in rows
